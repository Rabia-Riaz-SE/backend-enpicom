...
describe('DNAService', () => {
  ...
  describe('create', () => {
    ...
    it('should throw an error if DNA already exists', async () => {
      ...
      await expect(service.create(data.query)).rejects.toThrow(
        new HttpException(data.response.message, +data.response.statusCode),
      );
    });
  });
  ...
  describe('find', () => {
    ...
    it('should throw an error if levenshtein distance is not a number', async () => {
      ...
      await expect(service.find(data.query)).rejects.toThrow(
        new HttpException(data.response.message, +data.response.statusCode),
      );
    });
    ...
    it('should return all matched records matching DNA is empty, levenshtein distance is provided', async () => {
      ...
      const data: DNATestDataInterface<SearchDNADto, DNADto[]> =
        DNATestData.searchDNADtoWithEmptyDNA;
    ...
    it('should return all records matching DNA if levenshtein distance is not provided', async () => {
      const data: DNATestDataInterface<SearchDNADto, DNADto[]> =
        DNATestData.searchDNADtoWithNoLevenshtein;
